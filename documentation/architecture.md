...
- rotas de login, cadastro, confirmacao de e-mail por OTP, recuperacao de senha, Home e tela de analise parametrizada por `analysisId` (`lib/router.dart`, `lib/constants/routes.dart`);
...
- fluxo Home em MVP com presenter, widgets internos e criacao/navegacao para a tela de analise (`lib/ui/intake/widgets/pages/home_screen/`, `lib/ui/intake/widgets/pages/analysis_screen/`);
...
- implementacao REST concreta para `signIn`, `signInWithGoogle`, `signUp`, `verifyEmail`, `resendVerificationEmail`, `forgotPassword`, `verifyResetToken`, `resetPassword`, `fetchAccount`, `listAnalyses`, `createAnalysis`, `getAnalysis`, `createPetition`, `getAnalysisPetition`, `getPetitionSummary`, `summarizePetition`, `renameAnalysis`, `archiveAnalysis` e `generatePetitionUploadUrl` (`lib/rest/services/auth_rest_service.dart`, `lib/rest/services/intake_rest_service.dart`, `lib/rest/services/storage_rest_service.dart`);
- mapeadores REST para conta, sessao, analise, peticao, resumo de peticao e upload URL (`lib/rest/mappers/auth/`, `lib/rest/mappers/intake/`, `lib/rest/mappers/storage/`);
...
- drivers dedicados para selecao nativa de documento e upload/download de arquivo da peticao (`lib/drivers/document-picker-driver/`, `lib/drivers/storage/file_storage/`);
...
navega para a rota `/analyses/:analysisId` ao abrir ou criar uma analise.

Na tela de analise, o `AnalysisScreenPresenter` carrega a `analysis` atual, deriva o estado
visual a partir de `AnalysisStatusDto` e orquestra dois fluxos principais: upload da peticao
(`generatePetitionUploadUrl -> uploadFile -> createPetition`) e geracao do resumo
(`summarizePetition` com timeout de 60 segundos). O fluxo usa `DocumentPickerDriver` para
selecao nativa de PDF/DOCX, `FileStorageDriver` para upload direto ao storage e o gateway
REST de `intake` para persistencia da peticao, consulta do resumo, renomeacao e arquivamento
