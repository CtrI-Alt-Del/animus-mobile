...
class FirstInstancePdfGenerator {
  final AnimusPdfTheme _theme;

  const FirstInstancePdfGenerator({required AnimusPdfTheme theme})
    : _theme = theme;
    ...
    return pw.MultiPage(
      ...
      build: (pw.Context context) {
        return <pw.Widget>[
          ...
          pw.SizedBox(height: 8),
          _buildTextCard(report.caseSummary.legalIssue),
          pw.SizedBox(height: 12),
          _buildEyebrow('FATOS RELEVANTES'),
          pw.SizedBox(height: 6),
          _buildPlainList(report.caseSummary.keyFacts, ordered: true),
          pw.SizedBox(height: 12),
          _buildEyebrow('LEGISLAÇÃO APLICÁVEL'),
          pw.SizedBox(height: 6),
          _buildPlainList(report.caseSummary.relevantLaws),
        ];
      },
    );
  }
